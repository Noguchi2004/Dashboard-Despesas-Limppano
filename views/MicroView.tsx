                ...
                <div className="lg:col-span-2 bg-slate-800/40 border border-slate-700/50 rounded-xl p-5 h-64">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData}>
                             ...
                             <YAxis stroke="#94a3b8" tickFormatter={formatCompactCurrency} tick={{fontSize: 10}} axisLine={false} tickLine={false} />
                             <Tooltip 
                                 contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '8px' }}
                                 formatter={(val: number) => formatCurrency(val)}
                             />
                             <Legend />
                             <Line type="monotone" dataKey="current" name={`${selectedYear}`} stroke="#3b82f6" strokeWidth={3} dot={{r:3}} />
                             <Line type="monotone" dataKey="previous" name={`${compareYear}`} stroke="#94a3b8" strokeWidth={2} strokeDasharray="5 5" dot={false} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
           </div>
                   ...
                   <tbody className="divide-y divide-slate-800 text-sm">
                       {displayedData.length > 0 ? displayedData.map((row) => (
                           <tr key={row.id} className="hover:bg-slate-800/50 transition-colors group">
                               ...
                               <td className="px-6 py-4 text-center">
                                   {/* Logic to determine if drillable */}
                                   {(row.isSyntheticGroup || row.level < 5) && (
                                       <button 
