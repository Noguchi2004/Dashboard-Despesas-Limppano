...
const MicroView: React.FC<MicroViewProps> = ({ data, selectedYear, compareYear, selectedMonths }) => {
    ...
    return activeMonths.map((m) => {
      ...
      const calculateTotal = (year: number) => {
          ...
          if (isGroupView) {
              ...
              // MODE: Specific Account Drill Down (e.g. "4.1" or "4.1.02")
              
              // 1. Priority: Try to find the exact value for the account at its specific level.
              // This respects the user's selection: if they are at Level 3, we look for Level 3 data.
              const exactLevelSum = relevantItems
                .filter(d => d.accountCode === targetCode)
                .reduce((sum, item) => sum + item.amount, 0);

              // If the parent row has data, return it.
              if (Math.abs(exactLevelSum) > 0.01) {
                  return exactLevelSum;
              }

              // 2. Fallback: If the parent row is empty/zero (common in some Excel exports),
              // sum the analytical children (Level 5) to provide a valid chart.
              return relevantItems
                 .filter(d => d.category === targetCategory && 
                        d.accountCode.startsWith(targetCode) && 
                        d.level === 5)
                 .reduce((sum, item) => sum + item.amount, 0);
          }
      };
  ...
  const filteredData = useMemo(() => {
    ...
    // Aggregate Standard Children (Using specific logic for table display)
    // Here we trust the row values as we are drilling down into specific rows
    // But we still apply the same "fallback to children" logic if the row itself is zero?
    // Usually for deeper levels (3,4) if they are synthetic, they might be zero too.
    
    // Let's perform aggregation by unique code at this level
    ...
    const result = Array.from(aggregatedMap.values()).map(val => {
        ...
        // Apply fallback if this row is zero but it's not Level 5 (meaning it has children)
        if (Math.abs(finalAmount) < 0.01 && val.meta.level < 5) {
             // Find children in the FULL dataset (candidates might be filtered by level already)
             // We need to look at 'periodData' or a broader set
             const childrenSum = periodData
                .filter(d => 
                    d.year === selectedYear && 
                    d.dataType === 'REAL' &&
                    d.level === 5 && 
                    d.accountCode.startsWith(val.meta.accountCode)
                )
                .reduce((acc, curr) => acc + curr.amount, 0);
             
             if (Math.abs(childrenSum) > 0.01) finalAmount = childrenSum;
        }
