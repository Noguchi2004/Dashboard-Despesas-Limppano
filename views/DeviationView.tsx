...
const DeviationView: React.FC<DeviationViewProps> = ({ data, selectedYear, compareYear, selectedMonths }) => {
  ...
  // A. Deviation by Category (Level 1)
  const categoryDeviations = useMemo(() => {
    return calculateDeviations(
      analysisData.filter(d => d.level === 1), 
      'CATEGORY', 
      thresholds
    );
  ...
  const timelineData = useMemo(() => {
    ...
    return months.map(m => {
      const real = analysisData
        .filter(d => d.month === m.index && d.dataType === 'REAL' && d.category !== 'ROL' && d.level === 1)
        .reduce((sum, item) => sum + item.amount, 0);
      const budget = analysisData
        .filter(d => d.month === m.index && d.dataType === 'ORCADO' && d.category !== 'ROL' && d.level === 1)
        .reduce((sum, item) => sum + item.amount, 0);
