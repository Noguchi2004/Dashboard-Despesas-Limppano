...
import { 
  ...
  TrendingUp, Save, RotateCcw, ArrowRight, MinusCircle, Calendar, Target, X, ZoomIn, MousePointerClick
} from 'lucide-react';
import { 
...
const DeviationView: React.FC<DeviationViewProps> = ({ data, selectedYear, compareYear, selectedMonths }) => {
  ...
  // Updated: When attached to <Bar />, the first argument is the data entry object.
  const handleCategoryClick = (data: any) => {
    if (data && data.id) {
        setDrillDownCategory(data.id as CategoryType);
    }
  };
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-2">
        <div>
           <h2 className="text-3xl font-bold text-white flex items-center gap-2">
              Análise de Desvios
              {drillDownCategory && (
                  <span className="text-xl font-normal text-slate-400 flex items-center gap-1 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 shadow-lg">
             <div className="flex justify-between items-start mb-6">
                <h3 className="text-lg font-semibold text-white">Desvio por Categoria</h3>
                <div className="text-[10px] bg-blue-900/30 text-blue-300 px-2 py-1 rounded flex items-center gap-1 border border-blue-800">
                    <MousePointerClick size={10} /> Clique na barra para filtrar
                </div>
             </div>
             <div className="h-80 w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart 
                        data={categoryDeviations} 
                        layout="vertical" 
                        margin={{ left: 20 }}
                    >
                                    ...
                                    return (
                                        <div className="bg-slate-950 border border-slate-700 p-3 rounded shadow-xl">
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                <div className="pt-2 text-xs text-blue-400 font-bold border-t border-slate-800 mt-1">
                                                    Clique para detalhar contas
                                                </div>
                                            </div>
                                        </div>
                                    );
                        ...
                        <Bar 
                            dataKey="absDeviation" 
                            name="Desvio" 
                            radius={[0, 4, 4, 0]} 
                            onClick={handleCategoryClick}
                        >
                            {categoryDeviations.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={getStatusColorHex(entry.status)} cursor="pointer" />
                            ))}
                        </Bar>
                    </BarChart>
                </ResponsiveContainer>
