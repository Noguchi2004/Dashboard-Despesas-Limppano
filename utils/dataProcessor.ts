...
const parseMonth = (val: any): number => {
    ...
    // 1. Handle Numbers (Excel Serial Dates or integer months)
    if (typeof val === 'number') {
        // Standard Month Index 1-12
        if (val >= 1 && val <= 12) return Math.floor(val);
        
        // Excel Serial Date (> 20000 implies dates after ~1954)
        // This handles cases where 'Mês' column is actually a Date formatted to show just the month
        if (val > 20000) {
           // Convert Excel Serial to JS Date (UTC to avoid timezone shifts)
           // 25569 is the offset days from 1900-01-01 to 1970-01-01
    ...
    // 2. Handle Numeric Strings (e.g. "11", "05")
    const num = parseInt(str);
    if (!isNaN(num) && num >= 1 && num <= 12) return num;

    // 3. Handle Month Names (PT & EN)
    ...
    // 4. Handle Date Strings (e.g. "2025-11-01", "Nov/25")
    const d = new Date(str);
    if (!isNaN(d.getTime())) {
        return d.getMonth() + 1;
    }
    
    return 1; // Default to Jan if parsing fails completely
};
...
export const processRawData = (data: any[], type: DataType = 'REAL'): ProcessedExpense[] => {
  return data.map((row, index) => {
    // 1. Normalize Keys (Trim spaces from headers)
    const safeRow: any = {};
    Object.keys(row).forEach(key => {
        safeRow[key.trim()] = row[key];
    });

    // 2. Extract and Trim String Fields
    ...
    // 3. Robust Month/Year/Level Parsing
    ...
    // Robust Amount Parsing to prevent NaN
    ...
    if (typeof rawAmount === 'number' && !isNaN(rawAmount)) {
        ...
        // Handle strings, potentially with commas or currency symbols
        const strVal = String(rawAmount || '').trim();
        if (strVal) {
             let num = Number(strVal);
             if (isNaN(num)) {
                 // Try replacing comma with dot (PT-BR format)
    ...
    return {
      ...
      idCtactb: idCtactb, // Added ID_CTACTB
      accountCode: ctactb,
